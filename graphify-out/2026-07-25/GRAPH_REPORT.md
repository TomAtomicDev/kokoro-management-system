...
## Summary
- 2792 nodes · 7743 edges · 157 communities (131 shown, 26 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 77 edges (avg confidence: 0.77)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `06311432`
...
1. `nowIso()` - 109 edges
2. `Db` - 103 edges
3. `generateUuidV7()` - 72 edges
4. `validationError()` - 69 edges
...
Cohesion: 0.09
Nodes (32): RecipeEdge, topoOrderAffectedItems(), assertFiniteNonNegative(), assertSafeIntegerInput(), computePriceMargin(), computePriceSuggested(), listPriceHealth(), PriceHealthItemRow (+24 more)
...
Cohesion: 0.11
Nodes (21): RecordTransactionDialogProps, TransactionsTableProps, TransferDialogProps, WithdrawDialogProps, PurchaseDetailDrawerProps, PurchaseFormProps, PurchasesTableProps, CollectPaymentDialogProps (+13 more)
...
Cohesion: 0.10
Nodes (29): auditRoute, costingRoute, dashboardRoute, errorHandler(), healthRoute, ACTOR, onboardingRoute, productionRunsRoute (+21 more)
...
Cohesion: 0.12
Nodes (19): ACTOR, financeRoute, detectWacDrift(), WacDrift, getBalanceConsistencyMismatches(), listAccounts(), getStockConsistencyMismatches(), getStockValueTotal() (+11 more)
...
Cohesion: 0.12
Nodes (14): authRoute, ADR-0007, isLoginRateLimited(), recordFailedLoginAttempt(), rateLimited(), createDb(), getCookieValue(), login() (+6 more)
...
Cohesion: 0.08
Nodes (36): ACCOUNTS_KEY, filtersToQueryString(), RECEIVABLES_KEY, saleDetailKey(), SALES_ROOT_KEY, salesListKey(), useCollectPayment(), useDeleteSale() (+28 more)
...
Cohesion: 0.15
Nodes (14): BackupRunDetail, backupsRoute, ADR-0015, buildDailySnapshotUpsert(), buildJobRunInsert(), DailySnapshotValues, getLatestJobRun(), JobRunValues (+6 more)
...
## Knowledge Gaps
- **803 isolated node(s):** `PriceHealthItemRow`, `EXPECTED_TABLES`, `EXPECTED_VIEWS`, `fs`, `path` (+798 more)
...
- **Why does `nowIso()` connect `Finance API Route` to `Inventory Command Services`, `Auth Routes & Audit Log`, `Onboarding & Catalog API`, `Backups & Dashboard API`, `WAC Costing Engine`, `Production Runs API`, `Transaction & Purchase Drawers UI`, `Item Form & Onboarding Steps`, `Count & Exit Forms UI`, `Production Movements & Kardex`, `Costing Routes & Replacement Cost`, `SessionDetailDrawer.tsx`, `Production Run Table UI`, `Crypto Utils & CSRF`, `Event & Count Tables UI`, `Finance API & Schemas`, `api/backups.ts`, `Auth API & Smoke Tests`, `Security Audit Skill Phases`, `Replay Confirmation Hook`, `daily-snapshot.ts`, `movements.ts`, `Client-Side Attack Classes`?**
  _High betweenness centrality (0.024) - this node is a cross-community bridge._
- **Why does `Db` connect `movements.ts` to `Inventory Command Services`, `Auth Routes & Audit Log`, `Onboarding & Catalog API`, `Backups & Dashboard API`, `WAC Costing Engine`, `Finance & Count Dialogs UI`, `Transaction & Purchase Drawers UI`, `production-runs.test.ts`, `Item Form & Onboarding Steps`, `SalesRoute`, `Production Movements & Kardex`, `Crypto Utils & CSRF`, `api/backups.ts`, `Auth API & Smoke Tests`, `Security Audit Skill Phases`, `daily-snapshot.ts`, `Purchasing API & Schemas`, `Finance API Route`, `Client-Side Attack Classes`?**
  _High betweenness centrality (0.010) - this node is a cross-community bridge._
- **Why does `createDb()` connect `production-runs.test.ts` to `Inventory Command Services`, `Auth Routes & Audit Log`, `Backups & Dashboard API`, `WAC Costing Engine`, `Finance & Count Dialogs UI`, `production-runs-routes.test.ts`, `src/catalog.ts`, `Transaction & Purchase Drawers UI`, `Sessions Domain Service`, `Item Form & Onboarding Steps`, `Count & Exit Forms UI`, `api/backups.ts`, `Purchasing Route Tests`, `Auth API & Smoke Tests`, `Counts API & Schemas`, `Security Audit Skill Phases`, `Replay Confirmation Hook`, `Recipe Dependency Graph & Domain Errors`, `Exit/Purchase Route Tests`, `daily-snapshot.ts`, `Purchasing API & Schemas`, `movements.ts`, `Sessions Route Tests`, `Finance API Route`, `sales/api.ts`, `Client-Side Attack Classes`?**
  _High betweenness centrality (0.010) - this node is a cross-community bridge._
- **What connects `PriceHealthItemRow`, `EXPECTED_TABLES`, `EXPECTED_VIEWS` to the rest of the system?**
