-- KOK-071 vertical 2/4 (ADR-017): `items.replacement_cost` moves from REAL centavos per
-- milli-unit to INTEGER milli-centavos per WHOLE unit. The factor is 1,000,000: changing the
-- denominator to a whole unit is x1000 and milli-centavos supply the second x1000.
--
-- SQLite validates dependent views during a table rename, so every view that depends on `items`
-- is dropped before the recreate and restored after the final table shape is in place.
...
INSERT INTO `__new_items` ("id", "name", "kind", "category", "unit", "wac_mc", "replacement_cost_mc", "replacement_cost_updated_at", "sale_price", "min_stock_qty", "is_active", "notes", "created_at", "updated_at")
SELECT "id", "name", "kind", "category", "unit", "wac_mc", CAST(ROUND("replacement_cost" * 1000000) AS INTEGER), "replacement_cost_updated_at", "sale_price", "min_stock_qty", "is_active", "notes", "created_at", "updated_at" FROM `items`;
--> statement-breakpoint
...
SELECT
  ...
  i.wac_mc, i.replacement_cost_mc, i.sale_price, i.min_stock_qty, i.is_active,
...
SELECT
  i.id AS item_id, i.name, i.sale_price, i.wac_mc, i.replacement_cost_mc,
