-- KOK-122 / Doc 03 C-10: add ASSEMBLY_OUT for consumed components and ASSEMBLY_IN for the
-- stockable presentation/combo output. SQLite CHECK constraints require the standard table
-- recreation; dependent views are dropped first and recreated from their latest definitions.
...
CREATE TABLE `__new_stock_movements` (
	...
	CONSTRAINT "stock_movements_type_check" CHECK("__new_stock_movements"."type" IN ('PURCHASE_IN','PRODUCTION_IN','PRODUCTION_OUT','SALE_OUT','EXIT_OUT','ADJUST','OPENING_IN','ASSEMBLY_IN','ASSEMBLY_OUT'))
);
