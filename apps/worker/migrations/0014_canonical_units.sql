...
CREATE TABLE `__new_items` (
	...
	CONSTRAINT "items_unit_check" CHECK("__new_items"."unit" IN ('KG','L','M','UNIT'))
);
--> statement-breakpoint
INSERT INTO `__new_items`("id", "name", "kind", "category", "unit", "wac_mc", "replacement_cost_mc", "replacement_cost_updated_at", "sale_price_mc", "min_stock_qty", "is_unmetered", "is_active", "notes", "created_at", "updated_at") SELECT "id", "name", "kind", "category", "unit", "wac_mc", "replacement_cost_mc", "replacement_cost_updated_at", "sale_price_mc", "min_stock_qty", "is_unmetered", "is_active", "notes", "created_at", "updated_at" FROM `items`;--> statement-breakpoint
