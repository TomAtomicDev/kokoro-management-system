// Drizzle schema ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â mirrors migrations/0001_init.sql 1:1 (Doc 04 header rule), including every
// CHECK constraint, so `drizzle-kit generate` against this file reproduces the same DDL
// (Doc 04 Ãƒâ€šÃ‚Â§8: migrations are "generated by drizzle-kit"). Two things `drizzle-kit generate`
...
export const itemAliases = sqliteTable("item_aliases", {
  ...
  // Doc 04 Ãƒâ€šÃ‚Â§3.1 specifies `COLLATE NOCASE` for case-insensitive alias matching. This drizzle-orm
  // version's sqlite text() builder has no `.collate()` modifier, so it is hand-patched onto this
  // column directly in migrations/0001_init.sql ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â if you add drizzle-kit-generated migrations
...
export const recipes = sqliteTable(
  ...
  (t) => ({
    // Partial unique index: one default recipe per output item (Doc 04 Ãƒâ€šÃ‚Â§3.1).
...
export const sessionCosts = sqliteTable(
  "session_costs",
  {
    ...
    // Forward reference ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â financial_accounts is declared later in this file (Doc 04 Ãƒâ€šÃ‚Â§3.4).
...
export const sales = sqliteTable(
  "sales",
  {
    ...
    // Forward reference ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â custom_orders is declared later (circular FK, Doc 04 Ãƒâ€šÃ‚Â§3.3).
    customOrderId: text("custom_order_id").references((): AnySQLiteColumn => customOrders.id, {
      onDelete: "restrict",
    }),
...
// ============================================================================
// 4. Derived ledgers (Doc 04 Ãƒâ€šÃ‚Â§3.4)
// ============================================================================
...
// for every later entry ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â so an exit CAN move downstream WAC, and that correction must be
// bookable. Doc 04 Ãƒâ€šÃ‚Â§3.4 carries the same DDL.
//
// KOK-028 amendment: `trigger_event_type` further admits 'session'. Closing a PRODUCTION session
// (S-3) can recompute several production runs' `allocated_session_cost`/output unit cost at once ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â
...
// entry exactly like a backdated exit does (KOK-024 amendment above) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â the correction must be
// bookable against the sale that triggered it, closing the gap `docs/development/kok-030-sales-
// end-to-end.md` Ãƒâ€šÃ‚Â§2 documented (recordSale previously never called planCostingReplay because
...
// ============================================================================
// 5. System & observability (Doc 04 Ãƒâ€šÃ‚Â§3.5)
// ============================================================================
