...
export interface PriceHealthRowDto {
  ...
  /** Centavos per MILLI-unit (Doc 04 §2) — the raw stored value, same convention as `items.wac`. */
  wac: number;
  /** Centavos per MILLI-unit — same convention as `items.replacementCost`. */
  ...
  /** C-5 margin over `replacementCost` ("margen real") — the anti-decapitalization figure Doc 06
  ...
  /** `replacement_cost / (1 − min_margin_pct)` (Doc 07 SC-12), centavos per whole unit, rounded
   * half-up. `null` when `replacementCost` is 0 — nothing to mark up from yet (e.g. C-3 hasn't run). */
